...
function init(modules: { typescript: typeof import("typescript/lib/tsserverlibrary") }) {
    ...
    function create(info: ts.server.PluginCreateInfo): ts.LanguageService {
        ...
        const projectEventHandler: ts.server.ProjectServiceEventHandler = (event) => {
            ...
            case ts.server.ProjectLoadingStartEvent:
                ...
                if (event.data.project.getProjectName() === info.project.getProjectName()) {
            ...
            case ts.server.ProjectLoadingFinishEvent:
                ...
                // These events are triggered on all projects, but we only want to
                // handle the events for this project.
                event.data.project.getProjectName() === info.project.getProjectName()
                ) {
